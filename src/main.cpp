...
uint256 hashGenesisBlock("0x3b8b7a836df44a5401fb9be40f8c917a057a5a68913d2819799b60c1eb347547");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Madcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Madcoin Signed Message:\n";
...
{
    // Madcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 660000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 660000); // Madcoin: 660k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Madcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Madcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Madcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Madcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x59e40f118741ef51bb86cd15e9ef85e16364098e4821d5cfa304e463e5632ba6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "THE PEOPLE's MAD MONEY";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 666 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a3b4177b91fb27170aae0d55c50c530393ec0d5352a5f963b20bdfd14d29a5d6ecc0be9edbfa2543e09aeae91974ed2c7c3b3ac22ef1c4ba063552a4f5c8a7a7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1528728068;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086387391;

        if (fTestNet)
        {
            block.nTime    = 1528728051;
            block.nNonce   = 385639034;
        }
       ...
       if (false && block.GetHash() != hashGenesisBlock)
       {
        ...
        assert(block.hashMerkleRoot == uint256("0xc2f5fd5538579b4e2f86c23ccb7a007767035d4072216f779f7ac9449adef055"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf4, 0xc2, 0xb1, 0xd0 }; // Madcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MadcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MadcoinMiner(CWallet *pwallet)
{
    printf("MadcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running MadcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MadcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MadcoinMiner, pwallet));
}
